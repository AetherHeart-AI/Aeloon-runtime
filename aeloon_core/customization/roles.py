"""Python-defined agent roles and immutable startup snapshots."""
...
class RoleRegistry:
    ...
    @classmethod
    def discover(cls, project_root: Any) -> RoleRegistry:
        """Compatibility entry point returning the unified Catalog's Role registry."""

        from aeloon_core.customization.catalog import Catalog

        return Catalog.discover(project_root).roles
    ...
    @property
    def workers(self) -> Mapping[str, RoleSnapshot]:
        """Compatibility alias for the previous WorkerRegistry mapping."""
...
# Compatibility names for integrations that imported the previous Worker catalog types.
WorkerDefinitionError = RoleDefinitionError
WorkerRegistry = RoleRegistry
WorkerSnapshot = RoleSnapshot
