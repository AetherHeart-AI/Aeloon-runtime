"""Integration tests for composing the Harness feature packages."""
...
from aeloon_core.harness.agent.base import Role, snapshot_role
...
def test_master_always_exposes_ephemeral_worker_catalog(tmp_path: Path) -> None:
    ...
    registry = Catalog.discover(tmp_path).roles
    model = TestModel()
...
def test_worker_output_limit_respects_a_lower_model_setting(tmp_path: Path) -> None:
    ...
    workflow = master_harness_capabilities(
        ...
        roles=Catalog.discover(tmp_path).roles,
    )[1]
