...
def assert_contract(kind: str, name: str, value: object) -> None:
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    reference = manifest[f"{kind}s"][name]["payload" if kind == "event" else "result"]
    Draft202012Validator({"$ref": reference["$ref"], "$defs": manifest["$defs"]}).validate(value)
...
@pytest.mark.asyncio
async def test_rpc_v2_rejects_incompatible_handshakes(tmp_path: Path) -> None:
    ...
    try:
        ...
        with pytest.raises(RpcError) as wrong_version:
            await adapter.dispatch("system.handshake", {"protocol": "aeloon-rpc-v1"})
        assert wrong_version.value.code == "protocol_incompatible"
        ...
        assert handshake["core_version"] == "0.0.16"
        ...
        assert_contract("method", "system.handshake", handshake)
        catalog = await adapter.dispatch("catalog.get", {"workspace": str(tmp_path)})
        assert_contract("method", "catalog.get", catalog)
...
async def test_ui_thread_id_is_the_core_session_id(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert_contract("method", "session.create", created)
        fetched = await adapter.dispatch("session.get", {"session_id": thread_id})
        assert_contract("method", "session.get", fetched)
