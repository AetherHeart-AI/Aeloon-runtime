...
class CloudProviderConfig(BaseModel):
    ...
    proxy: str | None = None
    device_name: str = "Aeloon Runtime"
...
class Config(BaseModel):
    ...
    data_dir: Path = Field(default_factory=lambda: Path("~/.aeloon-runtime").expanduser())
...
def default_config_path() -> Path:
    raw = os.environ.get("AELOON_RUNTIME_CONFIG")
    return Path(raw).expanduser() if raw else Path("~/.aeloon-runtime/config.json").expanduser()
